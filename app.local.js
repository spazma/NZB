...
function renderHistoria() {
    ...
    if (!Array.isArray(hist) || hist.length === 0) {
        ...
        hist.slice(-historyVisibleCount).reverse().forEach(h => {
...
function showFullHistory() {
    ...
    let html = "<h3 style='margin-bottom:1.2em;'>Pełna historia oglądania:</h3><ol style='margin-bottom:2em;'>";
    shown.forEach(h => {
    ...
    setTimeout(() => {
        document.getElementById('clear-cache-btn-modal')?.addEventListener('click', () => window.closeModal());
        document.getElementById('remove-api-key-btn-modal')?.addEventListener('click', () => window.closeModal());
        document.querySelector('.modal-btn-danger')?.addEventListener('click', () => window.closeModal());
    }, 100);
}
...
function showDebug(msg, type="ok") {
    if (statusDebugActive && !(msg.includes("API:") || msg.includes("Ready"))) return;
